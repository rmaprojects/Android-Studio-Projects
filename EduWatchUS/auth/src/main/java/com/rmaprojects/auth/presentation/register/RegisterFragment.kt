...
class RegisterFragment: Fragment(R.layout.fragment_register) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch {
            viewModel.fetchSchoolUiEvent.collectLatest { event ->
                when (event) {
                    is FetchSchoolUiEvent.Error -> {
                        buildSnackbar(
                            binding.root,
                            event.message
                        ).show()
                    }
                    is FetchSchoolUiEvent.Success -> {
                        val items = event.data.map { it.schoolName }.toTypedArray()
                        binding.inputSchool.setSimpleItems(items)
                        binding.inputSchool.setOnItemClickListener { _, _, position, _ ->
                            binding.inputSchool.setText(items[position])
                            viewModel.setSelectedSchoolId(event.data[position].id)
                            if (viewModel.schoolId != null) viewModel.getStudents()
                        }
                    }
                }
            }
            ...
            viewModel.fetchStudentUiEvent.collectLatest { event ->
                when (event) {
                    ...
                    is FetchStudentUiEvent.Success -> {
                        val items = event.data.map { it.name }.toTypedArray()
                        binding.inputStudent.setSimpleItems(items)
                        binding.inputStudent.setOnItemClickListener { _, _, position, _ ->
                            binding.inputStudent.setText(items[position])
